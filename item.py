...
class Item:
    ...
    def drink(self):
        format_output("I don't think you can drink the {}.".format(self.name[0]))
...
class flask(Item):
    def __init__(self):
        Item.__init__(self)
        self.description = "Just an ordinary FLASK filled with some sort of CHEAP LIQUOR."
        self.name = ('flask',)
        self.takable = True
        ...
        def drink(self):
            f = open(path.join("saveData", "flask.ba"), "r")
            drinkTimes = int(f.readline().strip('\n'))

            if not drinkTimes > 10:
                format_output('You drink out of the FLASK. It tastes like CHEAP ALCOHOL. Gross...')
                drinkTimes += 1
                drinkTimes = str(drinkTimes)
                with open(path.join("saveData", "flask.ba"), "w+") as f:
                    f.write(drinkTimes)
            else:
                format_output('The flask is empty. You may no longer drink out of it.')

class haiku(Item):
    def __init__(self):
        Item.__init__(self)
        self.description = "a haiku written in blood"
        self.name = ('haiku')
        self.takable = False
